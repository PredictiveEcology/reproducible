...
system.time({
  suppressWarnings({
    map1 <- terra::project(ras, tmplate, method = "near") |> Cache(cachePath = tmpDir, notOlderThan = Sys.time())
  })
})

# faster the second time; improvement depends on size of object and time to run function
system.time({
  map2 <- terra::project(ras, tmplate, method = "near") |> Cache(cachePath = tmpDir)
})
...
outer(n = 2) |> Cache(cachePath = tmpdir1, notOlderThan = Sys.time())

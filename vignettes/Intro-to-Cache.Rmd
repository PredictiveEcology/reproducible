...
Similar to `archivist::cache`, there is some reliance on `digest::digest` to determine whether the arguments are identical in subsequent iterations.
...
toRemove <- if (getOption("reproducible.newAlgo", TRUE)) {
  unique(wholeCache[!onlyRecentlyAccessed], by = "artifact")$cacheId
} else {
  unique(wholeCache[!onlyRecentlyAccessed], by = "artifact")$artifact
}
...
```{r manual-cache}
# As of reproducible version 0.3.0, there is a new backend directly using DBI, not archivist

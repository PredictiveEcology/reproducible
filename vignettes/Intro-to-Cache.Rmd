...
This can be done in several ways. First, pass the function name to the first
argument of `Cache`, with subsequent arguments passed as if they were being
passed to that function. Second, the function call can be `quote`d. Thirs, 
(still experimental) wrapping the function call with `Cache`. See examples.
...
if (requireNamespace("terra")) {
  ras <- terra::rast(terra::ext(0, 100, 0, 100), vals = rep(1:2, length.out = 1e4), res = 1)
  terra::crs(ras) <- "+proj=lcc +lat_1=48 +lat_2=33 +lon_0=-100 +datum=WGS84"
  
  newCRS <- "+init=epsg:4326" # A longlat crs
  
  # No Cache
  system.time(map1 <- terra::project(ras, y = newCRS)) 
  
  # With Cache -- a little slower the first time because saving to disk
  system.time(map2 <- Cache(terra::project(ras, y = newCRS), cacheRepo = tmpDir,
                                             notOlderThan = Sys.time()))
  
  # vastly faster the second time
  opts <- options("reproducible.useMemoise" = FALSE) # make sure memoising is off (a RAM copy)
  system.time(map3 <- Cache(terra::project(ras, y = newCRS), cacheRepo = tmpDir))
  
  # Try with memoise for this example -- for simple cases, memoising may not be faster
  options("reproducible.useMemoise" = TRUE)
  system.time(suppressWarnings(map4 <- Cache(terra::project(ras, y = newCRS), cacheRepo = tmpDir)))
  options(opts)
  
  # All same, except foBr some Cache attributes and can't use base::all.equal directly
  allEqual(map1, map2) # TRUE
  allEqual(map1, map3, tolerance = 1e6) # TRUE (imprecision with terra::wrap, used by Cache)
  allEqual(map1, map4, tolerance = 1e6) # TRUE (imprecision with terra::wrap, used by Cache)
}
```
...
```{r}
clearCache(tmpDir, ask = FALSE) # just to make sure it is clear

ranNumsA <- Cache(rnorm, 10, 16, cacheRepo = tmpDir)

# All recover same Cached copy -- alternative ways of writing
ranNumsB <- Cache(rnorm, 10, 16, cacheRepo = tmpDir) 
ranNumsC <- Cache(quote(rnorm(n = 10, 16)), cacheRepo = tmpDir) 
ranNumsD <- Cache(rnorm(n = 10, 16), cacheRepo = tmpDir) 
...
if (requireNamespace("terra")) {
  ...
  notCached <- terra::project(ras, y = terra::crs(ras), res = 0.25)
  
  cached <- 
    Cache(terra::project(ras, y = terra::crs(ras), res = 0.25), cacheRepo = tmpDir)
  ...
  all.equal(notCached@ptr, reRun@ptr) ## TRUE # The attributes are slightly different due to Cache
}
```
...
# override even with different inputs ... obviously, this can go very wrong; use wisely!
...
In general, we feel that a liberal use of `Cache` will be part of a robust, re-usable and reproducible work flow.

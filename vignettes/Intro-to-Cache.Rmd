...
terra::crs(ras) <- "+proj=lcc +lat_0=50 +lat_1=60 +lon_0=-100 +datum=WGS84"

newCRS <- "epsg:4326" # A longlat crs
tmplate <- terra::rast(terra::ext(c(-100, -99.99587, 50, 50.00266)), nrows = 300, ncols = 300)
terra::crs(tmplate) <- newCRS

# No Cache
# use_gdal = FALSE is not necessary in most cases
system.time(map1 <- terra::project(ras, tmplate, method = "near", use_gdal = FALSE))
...
system.time({
  suppressWarnings({
    map1 <- terra::project(ras, tmplate, method = "near", use_gdal = FALSE) |> 
...
system.time({
  map2 <- terra::project(ras, tmplate, method = "near", use_gdal = FALSE) |> 
    Cache(cachePath = tmpDir)
})

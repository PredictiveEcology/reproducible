...
SuggestedPkgsNeeded <- c("terra", "sf")
...
lcc_wkt <- sf::st_crs("+proj=lcc +lat_1=48 +lat_2=33 +lon_0=-100 +datum=WGS84")$wkt

# Assign WKT to your raster
terra::crs(ras) <- lcc_wkt
newCRS <- "epsg:4326" # A longlat crs

# No Cache
system.time(suppressWarnings(map1 <- terra::project(ras, newCRS, use_gdal = FALSE))) # Warnings due to new PROJ
...
system.time({
  suppressWarnings({
    map1 <- terra::project(ras, newCRS, use_gdal = FALSE) |> 
      Cache(cachePath = tmpDir, notOlderThan = Sys.time()) 
  })
})

# faster the second time; improvement depends on size of object and time to run function
system.time({
  map2 <- terra::project(ras, newCRS, use_gdal = FALSE) |> 
...
notCached <- suppressWarnings(
  ...
  terra::project(ras, rasCRS, res = 5, use_gdal = FALSE)
)

cached <- suppressWarnings(
  # project raster generates warnings when run non-interactively
  # using quote works also
  terra::project(ras, rasCRS, res = 5, use_gdal = FALSE) |> Cache(cachePath = tmpDir)
)

...
setMethod(
  ...
  definition = function(object, .objects, length, algo, quick,
    ...
    if (is(object, "cluster")) {# can't get this class from parallel via importClass
...
setMethod(
  ...
  definition = function(object, .objects, length, algo, quick, classOptions) {
    ...
    simpleDigest <- TRUE
    if (!quick) {
      if (any(file.exists(object[1:6]))) { # only try first 6 elements; ok if < length 6 (returns NA; then FALSE)
        simpleDigest <- FALSE
      }}
    if (!simpleDigest) {

# `reproducible.sharedInputs` — Design Spec
...
**v4 changes:**
- `sharedInputs` is **read-write**. When a download is needed and
  `sharedInputs` is configured + writable, the file is downloaded into the
...
- Public `sharedInputsLs()` exposed now. (§7 closed)
- Legacy `<urlEncoded>.hash` files in `sharedInputs` are **deleted** after
  hydration to the sidecar format (matches destinationPath behavior). (§3.6)

**v3 changes:**
- `<sharedInputs>/CHECKSUMS.txt` is **dropped**. Shared dirs use file-keyed
...
- Rename: `reproducible.dataPath` → `reproducible.sharedInputs` (see §0).
- Resolution order revised so the sharedInputs search uses authoritative
...
A user-set option, `reproducible.sharedInputs`, that names one or more
directories on the local filesystem treated as a *shared read-through cache*
for files fetched by `prepInputs()` / `preProcess()`.

**Single sentence:** before downloading, look in `reproducible.sharedInputs`
...
| ------------------------------------- | ---------------- | ------- | ------- |
| `reproducible.sharedInputs`           | `NULL` \| `character(>=1)` | `NULL` | Directories to search before downloading. Searched in order; first hash-match wins. `NULL` disables the mechanism. |
| `reproducible.sharedInputsRecursive`  | `logical(1)`     | `FALSE` | If `TRUE`, search each path recursively. Applies to **all** elements of `reproducible.sharedInputs`. |
| `reproducible.dataPath`               | (deprecated)     | —       | Alias for `sharedInputs`. Emits a one-shot deprecation message per session. |
| `reproducible.dataPathRecursive`      | (deprecated)     | —       | Alias for `sharedInputsRecursive`. |
| `reproducible.inputPaths`             | (deprecated)     | —       | Older alias for `sharedInputs`. |
| `reproducible.inputPathsRecursive`    | (deprecated)     | —       | Older alias for `sharedInputsRecursive`. |
...
makes **one** filesystem search of `sharedInputs`. Pseudocode:

```
pp_resolve_inputs(ctx):

  # Step A: figure out the canonical filename(s) we are looking for.
  # We need this BEFORE searching sharedInputs, otherwise basename
  ...
  #   4. <sharedInputs>/<dp>/.repro/<canonicalFile>.json (shared sidecar)
  ...
  if file exists in destinationPath AND matches expectedHash:
    return ctx (already done; skip download AND skip sharedInputs search)

  # Step D: search sharedInputs ONCE. Build the candidate list and pick
  # the best match. Scan order: each dp in option order; within a dp,
  # recursive iff reproducible.sharedInputsRecursive.
  candidates = []
  for dp in reproducible.sharedInputs:
    for f in list_files(dp, recursive = reproducible.sharedInputsRecursive):
  ...
  for candidate in candidates:
    ...
    if expectedHash is None or h == expectedHash:
      ...
      log "hash mismatch in sharedInputs candidate; not using" (no delete)

  # Step F: nothing matched in sharedInputs. Fall through to pp_download.
  return ctx
...
pp_finalize_placement(downloadedFile, ctx):
  ...
  writableShared = first(dp in reproducible.sharedInputs where writable(dp))
  if writableShared is not None and downloadedFile not already in shared:
    ...
    # Either no sharedInputs configured, or none writable.
    leave downloadedFile in <destinationPath>/<canonicalFile>
    writeSidecar(<destinationPath>/.repro/<canonicalFile>.json, ...)
```

Key changes from today:
- **One filesystem traversal of `sharedInputs` per call.** Today the same dirs
...
- **`remoteHash` is first-class.** When the server gave us an etag/md5, we
  use that to validate sharedInputs candidates directly — no CHECKSUMS round
  trip required.
- **JSON sidecars replace `<url>.hash` files.** File-keyed at
  `<dir>/.repro/<filename>.json` (see §3.6). One format, two locations
  (destinationPath and sharedInputs). Existing `<urlEncoded>.hash` files are
  migrated transparently on first encounter.
- **No CHECKSUMS.txt in `sharedInputs`.** Sidecars are the only metadata.
...
- **`<dir>/.repro/<filename>.json` — file-keyed sidecars.** Machine-managed.
  Written in both `destinationPath` and `sharedInputs`. The *cache of
  observations* — what hash this file most recently had, where it came from,
  when it was saved.

**No CHECKSUMS.txt is written to or read from `sharedInputs`.** Removed
because:
...
**Legacy reading (one-time migration):** if a sharedInputs dir has a
...
to the legacy file. After hydration, sharedInputs is sidecar-only. Emit a
...
4. Else read `<sharedInputs>/<dp>/.repro/<canonicalFile>.json` (shared
   sidecar, in `sharedInputs` order).
...
- We still scan `sharedInputs` for files whose computed hash matches the
...
- Write/update `<sharedInputs>/<dp>/.repro/<canonicalFile>.json` for the
...
Multiple R sessions writing to the same `sharedInputs` dir is the **expected**
...
- **Exclusive (write) lock** around: copying or hardlinking a file *into*
  sharedInputs; writing or updating its sidecar; running the legacy
  CHECKSUMS hydration.
- **Shared (read) lock** around: reading the sidecar, hardlinking the file
  *out* (i.e., from sharedInputs into destinationPath).
...
**Lock dir not creatable** (read-only sharedInputs): one-shot warning;
...
`destinationPath` or one of `sharedInputs`. The `.repro/` subdir is hidden
...
When a download is required and `sharedInputs` is configured with at least
one writable directory:

1. Download to a tmp file.
2. `file.rename()` (atomic POSIX) the tmp file to the canonical path inside
   the **first writable** `sharedInputs` entry, under exclusive lock.
3. `hardLinkOrCopy()` from there into `destinationPath`.
4. Write sidecars in both locations.

When `sharedInputs` is unset or has no writable entry, the download lands in
...
**Why this matters:** users with `sharedInputs` set on cluster NFS get
...
- `sharedInputsLs(path = NULL, hash = TRUE)` — inspect what is in the
  ...
  "reproducible.sharedInputs")`. If `hash = FALSE`, sidecar fields are
  filled but no hash recompute is triggered for files lacking a sidecar.
- `sharedInputsRefresh(path = NULL, deleteOrphans = FALSE)` — rebuild
...
Test file: `tests/testthat/test-sharedInputs.R`. Each row → one `test_that()`
block.
...
| # | Setup | Assertion |
|---|---|---|
| P1 | option unset | `.getSharedInputs()` returns `NULL` |
| P2 | `reproducible.sharedInputs = "/x"` | returns `"/x"` |
| P3 | `reproducible.sharedInputs = c("/a","/b")` | returns `c("/a","/b")` |
...
| P6 | both `sharedInputs` and `dataPath` set | new wins; deprecation message NOT emitted |
| P7 | all three set (`sharedInputs`, `dataPath`, `inputPaths`) | `sharedInputs` wins; no deprecation message |
| P8 | `reproducible.sharedInputs = ""` | error: empty path |
| P9 | `reproducible.sharedInputs = c("/x","/x")` | de-duplicated to `"/x"` |
| P10 | `reproducible.sharedInputs = "/x"`, destinationPath = `"/x"` | search disabled (drop-self) |
| P11 | `reproducible.sharedInputsRecursive` unset | returns `FALSE` |
| P12 | `reproducible.sharedInputsRecursive = TRUE` | returns `TRUE` |
...
|---|---|---|---|
| H1 | empty destinationPath; file present in `sharedInputs` with matching hash | `prepInputs(url=..., destinationPath, targetFile=foo)` | no download attempted; file in destinationPath; both files share inode (hardlink) when same device |
| H2 | as H1, but cross-device | as H1 | no download; file present; copied (not hardlinked); message about copy fallback emitted once |
| H3 | as H1, recursive disabled, file is in subdir of `sharedInputs` | as H1 | download happens (file not found at top level) |
| H4 | as H3 but `sharedInputsRecursive = TRUE` | as H1 | no download; file linked from subdir |
| H5 | two `sharedInputs` entries, file only in second | as H1 | found in second path; no download |
| H6 | two `sharedInputs` entries, file in both with different mtimes, both valid | as H1 | first wins (deterministic order); assert which one was linked by inode |
| H7 | file present in destinationPath AND sharedInputs, both valid | as H1 | neither download nor link; existing destinationPath copy used; `sharedInputs` not even traversed |
| H8 | file present in destinationPath but with stale `<url>.hash` matching old version, sharedInputs has correct version | as H1 | sidecar mismatch → search proceeds; sharedInputs match wins; sidecar refreshed |

### 5.3 Resolution — sad paths

| # | Pre-state | Action | Expected |
|---|---|---|---|
| S1 | file present in `sharedInputs` but hash mismatch | `prepInputs(...)` | the bad file is NOT used, NOT deleted; informative message names the file and the dir; download proceeds normally |
| S2 | file present in `sharedInputs` but no expected hash anywhere (no CHECKSUMS, no remote, no sidecar) | `prepInputs(...)` | one-shot warning; basename match accepted; sidecar created with computed hash for future runs |
| S3 | `sharedInputs` set to a path that doesn't exist | `prepInputs(...)` | path is created; lookup is empty; download proceeds; no error |
| S4 | `sharedInputs` set to a path with no read permission | `prepInputs(...)` | warning; downloads proceed; no crash |
| S5 | filename collision: two files with same basename in recursive sharedInputs, only one matches expected hash | `prepInputs(...)` | matching one is used; non-matching one is ignored without error |
...
| # | Pre-state | Action | Expected |
|---|---|---|---|
| C1 | first-ever download, sharedInputs set, no sidecars anywhere | `prepInputs(...)` | project CHECKSUMS in destinationPath gains entry; sidecars written in both `<dest>/.repro/<file>.json` and `<dp>/.repro/<file>.json` |
| C2 | second project, same sharedInputs, sidecar present in shared | `prepInputs(...)` from a fresh destinationPath | hits sharedInputs via shared sidecar; project CHECKSUMS in destinationPath gains entry; destinationPath sidecar created; shared sidecar unchanged |
| C3 | sharedInputs read-only (cannot create `.repro/`) | `prepInputs(...)` | destinationPath gets project CHECKSUMS + sidecar; shared sidecar skipped with one-shot warning |
...
| C8 | sharedInputs dir contains legacy `<urlEncoded>.hash` files but no `.repro/` | first `prepInputs(...)` | one-shot info message about hydration; sidecars written; legacy `.hash` files **deleted** after successful migration; subsequent calls use sidecars only |
| C9 | sharedInputs dir contains legacy `CHECKSUMS.txt` but no `.repro/` and no data files | first `prepInputs(...)` | hydration runs, no sidecars written (no adjacent files), no error; subsequent calls behave as empty shared |
...
| # | Pre-state | Action | Expected |
|---|---|---|---|
| T1 | `targetFile = "foo.tif"` supplied; sharedInputs has `foo.tif` matching CHECKSUMS | `prepInputs(url, targetFile="foo.tif")` | NO HEAD request issued; sharedInputs match wins |
...
| T3 | `targetFile = NULL`, URL ends in random hash (no extension); server returns `Content-Disposition: filename="foo.tif"` | `prepInputs(url)` | one HEAD; canonicalFile = `"foo.tif"`; sharedInputs searched with the right name |
| T4 | as T3 but content-disposition is missing; URL fallback gives a wrong name | `prepInputs(url)` | sharedInputs miss (correct: we genuinely can't tell); download proceeds |
| T5 | Google Drive URL, `targetFile = NULL` | `prepInputs(url)` | `getRemoteMetadata` uses drive_get; canonicalFile from `file$name`; remoteHash = md5; sharedInputs match by hash even if basename differs |
...
| # | Pre-state | Action | Expected |
|---|---|---|---|
| R1 | server returns etag `"abc"`; sharedInputs has `foo.tif` whose computed sha matches `"abc"` (after algo normalization) | `prepInputs(url, targetFile="foo.tif")` | linked from sharedInputs; no full download |
| R2 | server returns etag `"abc"`; sharedInputs has `foo.tif` whose hash is `"def"` | `prepInputs(...)` | hash mismatch logged (S1 path); download proceeds |
| R3 | no remote available (offline); `<destinationPath>/.repro/<file>.json` from prior run says `"abc"`; sharedInputs has matching file | `prepInputs(...)` | linked from sharedInputs using destinationPath sidecar's hash (Step B item 2) |
| R4 | sharedInputs has matching JSON sidecar; no other hash source | `prepInputs(...)` | linked; sidecar copied to destinationPath under exclusive lock |
...
| # | Pre-state | Action | Expected |
|---|---|---|---|
| B1 | code from before this refactor that sets `options(reproducible.inputPaths = ...)` | `prepInputs(...)` | works identically to setting `reproducible.sharedInputs`; one deprecation message per session |
| B2 | code that sets `options(reproducible.dataPath = ...)` (intermediate name) | `prepInputs(...)` | works; one deprecation message per session |
| B3 | `sharedInputs` and `dataPath` both set with different values | `prepInputs(...)` | `sharedInputs` wins; deprecated option ignored silently |

### 5.8 Integration with prepInputs/Cache

| # | Pre-state | Action | Expected |
|---|---|---|---|
| I1 | `Cache(prepInputs(...))` with cold cache, sharedInputs populated | first call | cache miss; prepInputs runs; uses sharedInputs; cache filled |
| I2 | as I1, second call | second call | cache hit; sharedInputs not consulted (no I/O) |
| I3 | `Cache(prepInputs(...))` cold cache, sharedInputs empty + writable | first call | downloads; file lands in sharedInputs; hardlinked into destinationPath; cache filled |
...
| # | Pre-state | Action | Expected |
|---|---|---|---|
| W1 | sharedInputs set + writable; file not in either dir; no Cache | `prepInputs(url, destinationPath)` | downloaded; physical file lives in sharedInputs; destinationPath has a hardlink (same inode); sidecars in both |
| W2 | sharedInputs set + writable; same call from a fresh second project | `prepInputs(...)` for the same url to a different destinationPath | NO download (W1's file picked up via sharedInputs); two destinationPaths now share one inode in sharedInputs; sidecar count = 3 (1 shared + 2 dest) |
| W3 | sharedInputs unset | `prepInputs(...)` | physical file in destinationPath; no shared activity (today's behavior preserved exactly) |
| W4 | sharedInputs set but read-only | `prepInputs(...)` | physical file in destinationPath; one-shot warning; sidecar in destinationPath only |
| W5 | sharedInputs has multiple entries; first writable, second read-only | `prepInputs(...)` | physical file in first; second untouched |
| W6 | sharedInputs has multiple entries; first read-only, second writable | `prepInputs(...)` | physical file in second; first read-checked but not written |
| W7 | cross-device sharedInputs (file.link fails) | `prepInputs(...)` | physical file in sharedInputs; copy (not hardlink) into destinationPath; one-shot warning naming the cost; **invariant relaxed** (now two physical copies — document explicitly) |
| W8 | sharedInputs same filesystem as destinationPath; sufficient free space | `prepInputs(...)` for a 1 GB fixture | exactly one inode used; assert `file.info(...)$nlinks == 2` |
| W9 | rename from tmp into sharedInputs fails (full disk in shared, dest on different FS) | `prepInputs(...)` | fall back to leaving file in destinationPath; one-shot warning; sidecar in destinationPath; no zero-byte file in shared |

### 5.10 Performance smoke

| # | Pre-state | Action | Expected |
|---|---|---|---|
| Q1 | sharedInputs with 10,000 unrelated files, recursive=FALSE | `prepInputs(...)` for one needed file | finishes in < 1s on typical disk; assert via `system.time` ceiling |
| Q2 | sharedInputs with deep nested tree, recursive=TRUE | `prepInputs(...)` | finishes in < 5s; primarily a regression guard |
...
| M2 | sharedInputs has `<urlEncoded>.hash` files | first `prepInputs(...)` reading from this dir | sidecars created; legacy `.hash` files **deleted from sharedInputs** under exclusive lock; message names count |
| M3 | sharedInputs has both `CHECKSUMS.txt` and `<urlEncoded>.hash`; entries don't fully overlap | first `prepInputs(...)` | hydration reads both, prefers `<urlEncoded>.hash` for entries that exist in both (it has the more authoritative remote hash); union written to sidecars |
...
| # | Pre-state | Action | Expected |
|---|---|---|---|
| L1 | two R sessions call `prepInputs(...)` for the same file simultaneously, both finding the file in sharedInputs | both call `prepInputs(...)` (use `callr::r_bg` × 2 or `parallel::mclapply`) | both succeed; destinationPaths each contain a valid file; sidecars consistent; no partial files |
| L2 | one writer copying a file *into* sharedInputs (slow `file.copy`, simulated with a hook), one reader trying to link the same file out | concurrent | reader either waits until writer finishes (acquires shared lock after exclusive released) or sees no file (writer hasn't started rename yet); never sees a partial file |
...
| L5 | `filelock` not installed | `prepInputs(...)` to a sharedInputs dir | one-shot warning at first lock attempt; operation proceeds without locking; rename-based atomicity still protects readers |
| L6 | `<sharedInputs>/.repro/locks/` not creatable (read-only mount) | `prepInputs(...)` | one-shot warning; reads proceed without lock; writes to shared sidecar are skipped; destinationPath sidecar still written |
...
### 5.13 Public API: `sharedInputsLs()` and `sharedInputsRefresh()`

| # | Pre-state | Action | Expected |
|---|---|---|---|
| A1 | sharedInputs unset | `sharedInputsLs()` | returns 0-row data.frame with the expected columns; no error |
| A2 | sharedInputs set, dir empty | `sharedInputsLs()` | 0-row data.frame |
| A3 | sharedInputs has 3 files with sidecars | `sharedInputsLs()` | 3 rows; columns populated from sidecars; no hash recompute |
| A4 | sharedInputs has 3 files, 2 with sidecars, 1 without; `hash = TRUE` | `sharedInputsLs()` | 3 rows; missing sidecar's hash computed and *also* written to disk under exclusive lock |
| A5 | as A4 but `hash = FALSE` | `sharedInputsLs(hash = FALSE)` | 3 rows; missing-sidecar row has `NA` hash; no recompute, no disk writes |
| A6 | sidecar exists but adjacent data file is missing (orphan) | `sharedInputsRefresh(deleteOrphans = FALSE)` | sidecar left in place; 0 rows reported as removed |
| A7 | as A6 with `deleteOrphans = TRUE` | `sharedInputsRefresh(deleteOrphans = TRUE)` | orphan sidecar deleted; 1 row reported |
| A8 | data file exists but no sidecar | `sharedInputsRefresh()` | sidecar created; row reported as added |
| A9 | data file exists with sidecar but adjacent file's hash no longer matches sidecar (manual replacement) | `sharedInputsRefresh()` | sidecar updated with new hash; row reported as repaired |
| A10 | `path` arg overrides option | `sharedInputsLs("/explicit/path")` | option ignored; explicit path scanned |
...
- Two new exported functions: `sharedInputsLs()`, `sharedInputsRefresh()`
  (§3.8).
- Internal helpers stay `.`-prefixed: `.readSidecar`, `.writeSidecar`,
  `.sidecarPath`, `.withFileLock`, `.hydrateLegacyChecksums`,
  `.normalizeSharedInputs`, `.resolveExpectedHash`, `.getSharedInputs`,
  `.getSharedInputsRecursive`, `.placeDownloadedFile`.
- NEWS.md entry highlights:
  - rename `reproducible.dataPath` → `reproducible.sharedInputs`;
  - shared CHECKSUMS.txt no longer maintained (sidecars instead);
  - sharedInputs now auto-populates on download (single-physical-copy
    invariant, §3.7);
  - locking via `filelock` (Suggests);
  - new exports `sharedInputsLs()` and `sharedInputsRefresh()`;
  - mismatched files in sharedInputs now logged, not silently skipped.
...
- ~~**Naming.**~~ Adopted `reproducible.sharedInputs`. (§0)
...
- ~~**Legacy `<urlEncoded>.hash`.**~~ Migrated to sidecar format on first
  encounter; deleted in both destinationPath and sharedInputs after
  successful migration. (§3.6, §5.11)
- ~~**Auto-population.**~~ sharedInputs is read-write. On download, place
...
- ~~**Public `sharedInputsLs()`.**~~ Yes, this iteration. Plus
  `sharedInputsRefresh()` for managing externally-populated dirs. (§3.8,
  §5.13)

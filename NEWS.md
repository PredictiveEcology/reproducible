...
* tweaks to how hashes are calculated. This will cause existing caches to not work correctly. To allow for a transition period, the "new" algorigthm is not default, but can be set with `options("reproducible.useNewDigestAlgorithm" = TRUE)`. There is a warning of this upcoming change.

...
test_that("prepInputs doesn't work (part 3)", {
  testInit(c("sf", "terra"), needGoogleDriveAuth = FALSE)

  silent <- capture.output({
  studyArea = {
    targetCRS <- "+proj=lcc +lat_0=0 +lon_0=-95 +lat_1=49 +lat_2=77 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs"
    ...
    nwt <- reproducible::prepInputs(url = "https://www12.statcan.gc.ca/census-recensement/2021/geo/sip-pis/boundary-limites/files-fichiers/lpr_000a21a_e.zip",
                                    projectTo = targetCRS) |>
      suppressWarningsSpecific(falseWarnings = "attribute variables are assumed to be spatially constant")
  ...
  ecoregionLayer <- reproducible::prepInputs(
    targetFile = "ecodistricts.shp",
    archive = reproducible::asPath("ecodistrict_shp.zip"),
  ...
  rr <- terra::rast(terra::ext(terra::vect(studyArea)), resolution = 250)
  sa <- terra::rasterize(studyArea, rr)
  ec <- terra::rasterize(ecoregionLayer, rr)

  testthat::expect_equal(0, sum(sa[is.na(ec[])], na.rm = TRUE)) # should be zero # it is 244293 in the wrong case

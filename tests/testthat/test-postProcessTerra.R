test_that("testing terra", {
  ...
  if (requireNamespace("terra")) {
    ...
    ### factors #############
    for (cls in c("Raster", "SpatRaster")) {
      xFact <- terra::deepcopy(xCut)
      if (identical(cls, "Raster")) {
        if (!requireNamespace("raster"))
          next
        xFact <- raster::raster(xFact)
      }

      levels(xFact) <- data.frame(ID = 0:4, vals = LETTERS[1:5])
      expect_true(terra::is.factor(xFact))
      xFactPost <- postProcessTerra(xFact, cropTo = valbers, maskTo = valbers, method = "near")
      xFactPost1 <- postProcess(xFact, studyArea = valbers, method = "near")
      expect_true(all.equal(xFactPost1, xFactPost))
      expect_true(terra::is.factor(xFactPost))
      expect_true(terra::is.factor(xFactPost1))
      expect_true(all.equal(terra::levels(xFactPost1), terra::levels(xFact)))
      expect_true(is(xFactPost, cls))
      expect_true(is(xFactPost1, cls))

      # test `useSAcrs`
      xFactPost <- postProcessTerra(xFact, to = valbers, method = "near")
      xFactPost1 <- postProcess(xFact, studyArea = valbers, useSAcrs = TRUE, method = "near")
      expect_true(all.equal(xFactPost1, xFactPost))
      expect_true(terra::is.factor(xFactPost))
      expect_true(terra::is.factor(xFactPost1))
      expect_true(all.equal(terra::levels(xFactPost1), terra::levels(xFact)))

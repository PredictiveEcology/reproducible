test_that("test miscellaneous unit tests cache-helpers", {
  ...
  expect_silent({
    a1 <- Cache(rnorm, 1, cacheRepo = tmpdir)
  })
  expect_message({
    a2 <- Cache(rnorm, 1, cacheRepo = tmpdir)
  })
  expect_message({
    a3 <- Cache(rnorm, 1, cacheRepo = tmpdir)
  })
  ...
  expect_silent({
    r1 <- Cache(raster, extent(0, 1, 0, 1), res = 1, vals = 1)
  })
  expect_message({
    r1 <- Cache(raster, extent(0, 1, 0, 1), res = 1, vals = 1)
  })
  ...
  expect_silent({
    r2 <- Cache(writeRaster, r1, file = tmpfile[1], overwrite = TRUE, quick = TRUE,
  ...
  expect_silent({
    r2 <- Cache(writeRaster, r1, file = tmpfile[2], overwrite = TRUE, quick = TRUE, cacheRepo = tmpdir)
  })
  expect_message({
    r2 <- Cache(writeRaster, r1, file = tmpfile[2], overwrite = TRUE, quick = TRUE, cacheRepo = tmpdir)
  })
